...
import { ArrowLeft, Save, Calendar, UserCircle, Sprout, Hash, Phone, MapPin, ChevronDown, Check } from 'lucide-react';
import { getSampleDetails, submitLabData } from '@/app/employee-v2/actions/lab';
    ...
    <div className="relative" ref={dropdownRef}>
        ...
        <button
           ...
           className={`w-full bg-white border flex items-center justify-between text-left text-sm font-bold rounded-2xl px-4 py-4 shadow-sm transition-all duration-200 ${
              isOpen ? 'border-purple-500 ring-2 ring-purple-100' : 'border-slate-200 text-slate-900'
           }`}
        >
           <span className="truncate">{selectedOption?.label || 'Select...'}</span>
           <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180 text-purple-600' : ''}`} />
        </button>
...
export default function SampleEntryPage({ params }: { params: Params }) {
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FDFCF8] p-5 text-center">
        ...
        <button onClick={() => router.back()} className="px-4 py-2 bg-slate-200 rounded-lg font-bold">Go Back</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FDFCF8] pb-10 font-sans">
      ...
      <div className="bg-white px-4 py-3 flex items-center justify-between shadow-sm sticky top-0 z-20">
         <div className="flex items-center gap-3">
            <button onClick={() => router.back()} className="text-slate-600 active:scale-95 transition-transform">
               <ArrowLeft className="w-6 h-6" />
            </button>
            <div>
               <h1 className="text-lg font-black text-slate-900 leading-tight">{data.farmer_name}</h1>
      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
          ...
          <div className="bg-[#FFF5F7] rounded-[24px] p-5 shadow-sm border border-red-50 relative overflow-hidden">
             <div className="grid grid-cols-2 gap-y-4 gap-x-2 relative z-10">
                ...
                <div>
                   <label className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Lot No</label>
                   <div className="text-sm font-black text-slate-800 flex items-center gap-1">
                      <Hash className="w-3 h-3 text-slate-400"/> {data.lot_no || 'N/A'}
                   </div>
                </div>

                <div>
                   <label className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Mobile</label>
                ...
                <div>
                   <label className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Village</label>
                   <div className="text-sm font-black text-slate-800 flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-slate-400"/> {data.village_name}
                   </div>
                </div>

                <div>
                   <label className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Location</label>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
             ...
             <div className="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_-10px_rgba(147,51,234,0.15)] border border-purple-100">
                ...
                <div className="space-y-4">
                    ...
                    <CustomDropdown 
                        ...
                        onChange={(val: string) => setFormState(prev => ({...prev, location: val}))}
                    />

                    {/* Moisture & Purity Grid */}
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-1.5 block">ભેજ (%)</label>
                            <input 
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-1.5 block">શુદ્ધતા (%)</label>
                            <input 
                                name="sample_purity"
                                type="number" 
                                step="0.01"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {/* Dust Percentage */}
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-1.5 block">કચરો (Dust %)</label>
                            <input 
                        ...
                        <div>
                           <CustomDropdown 
                              ...
                              onChange={(val: string) => setFormState(prev => ({...prev, color: val}))}
                           />
                        </div>
                    </div>
                    ...
                    <CustomDropdown 
                        ...
                        onChange={(val: string) => setFormState(prev => ({...prev, nonSeed: val}))}
                    />

                    {/* Remarks */}
                    <div>
                        <label className="text-xs font-bold text-slate-500 mb-1.5 block">રિમાર્ક (વૈકલ્પિક)</label>
                        <textarea 
                           name="remarks"
             ...
             <div className="bg-white rounded-[24px] p-5 shadow-sm border border-slate-100 opacity-70 grayscale">
                ...
                <input 
                   type="text"
                   value={data.price_per_20kg ? `₹ ${data.price_per_20kg}` : 'Pending'}
                   disabled
