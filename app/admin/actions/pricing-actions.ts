 ...
 */
export async function getCyclesPendingVerification(): Promise<CycleForPriceApproval[]> {
  noStore();
  try {
    const result = await sql`
      SELECT
...
export async function submitTemporaryPrice(formData: FormData) {
  ...
  try {
    ...
    `;
    revalidatePath('/admin-v2/dashboard');
    return { success: true };
  } catch (error) {
...
export async function verifyAndFinalizePrice(formData: FormData) {
  ...
  if (!validated.success) return { success: false, message: 'Invalid final price.' };
