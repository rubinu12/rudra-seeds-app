...
export function TestForm({ landmarks, seedVarieties }: { landmarks: Landmark[], seedVarieties: SeedVariety[] }) {
  ...
  const handleLandmarkChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    console.log(`[CLIENT LOG] User is selecting Landmark. Timestamp: ${Date.now()}`);
    setLandmarkId(e.target.value);
    // This log should appear instantly after the one above.
    console.log(`[CLIENT LOG] Landmark state updated in browser. New value: ${e.target.value}. Timestamp: ${Date.now()}`);
  };

  const handleVarietyChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    console.log(`[CLIENT LOG] User is selecting Variety. Timestamp: ${Date.now()}`);
    setSeedId(e.target.value);
    console.log(`[CLIENT LOG] Variety state updated in browser. New value: ${e.target.value}. Timestamp: ${Date.now()}`);
  };
  ...
  return (
    <div className="p-8 max-w-lg mx-auto bg-white rounded-lg shadow-md">
      ...
      <p className="mt-2 text-gray-600">This page isolates the dropdowns to diagnose lag. Check your browser and terminal consoles for logs.</p>
      
      <form action={formAction} className="mt-8 space-y-6">
        <div>
          <label htmlFor="landmark_id" className="block text-sm font-medium text-gray-700">Landmark (Loaded: {landmarks.length})</label>
          <select 
            id="landmark_id" 
            name="landmark_id"
            value={landmarkId}
        ...
        <div>
          <label htmlFor="seed_id" className="block text-sm font-medium text-gray-700">Seed Variety (Loaded: {seedVarieties.length})</label>
          <select 
            id="seed_id" 
            name="seed_id"
            value={seedId}
