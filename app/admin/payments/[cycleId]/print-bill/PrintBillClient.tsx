...
import type { FarmerPaymentDetails, StoredChequeDetail } from '@/lib/payment-data';
...
export default function PrintBillClient({ billData }: PrintBillClientProps) {
    ...
    const chequeNumbers = billData.cheque_details.map(c => c.cheque_number).join(', ');

    return (
        <>
            {/* Controls Section */}
            <div className={`controls p-4 max-w-4xl mx-auto my-4 border rounded shadow bg-white print:hidden ${isPrinting ? 'invisible' : ''}`}>
                <h2 className="text-xl font-semibold mb-4 text-center">Farmer Payment Bill / Consent</h2>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                     <Link href={`/admin/payments/${billData.crop_cycle_id}/process`} className="btn bg-gray-200 text-gray-700 py-2 px-4 rounded hover:bg-gray-300 flex items-center justify-center gap-2">
                        <ArrowLeft size={18}/> Back to Payment
                    </Link>
                    <button onClick={handlePrint} className="btn bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 flex items-center justify-center gap-2">
                        <Printer size={18}/> Print Bill
                    </button>
                    <Link href={`/admin/payments/${billData.crop_cycle_id}/print-cheque`} className="btn bg-green-600 text-white py-2 px-4 rounded hover:bg-green-700 flex items-center justify-center gap-2">
            ...
            <div className="bill-container">
                <div className="bill-a4">
                    ...
                    <div className="text-center mb-4 font-bold bill-header">
                        ...
                        <p className="text-md mt-2 underline">** ખેડૂત બીજ ઉત્પાદન ચુકવણી પત્ર / સંમતિ પત્ર **</p>
                    </div>
                    ...
                    <div className="flex justify-between mb-3 text-sm bill-info">
                        ...
                        <span>ગાડી નં./તારીખ: {billData.vehicle_number || 'N/A'} / {dispatchDate}</span>
                    </div>
                    ...
                    <table className="w-full border-collapse border border-black mb-4 text-sm bill-table">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td className="border border-black p-1 text-right">{man.toFixed(2)}</td>
                                <td className="border border-black p-1 text-right">₹ {ratePerMan.toFixed(2)}</td>
                                <td className="border border-black p-1 text-right">₹ {formatRupees(grossAmount)}</td>
                                <td className="border border-black p-1 text-right">₹ {formatRupees(deduction)}</td>
                                <td className="border border-black p-1 text-right">₹ {formatRupees(netAmount)}</td>
                            </tr>
                            <tr>
                                <td colSpan={6} className="border border-black p-1 text-right font-bold">ફાઇનલ રકમ (₹):</td>
                                <td className="border border-black p-1 text-right font-bold">₹ {formatRupees(netAmount)}</td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div className="mb-2 text-sm">નીચે મુજબના ચેક દ્વારા ચુકવણી કરેલ છે:</div>
                    <table className="w-full border-collapse border border-black mb-4 text-sm bill-table">
                        <thead>
                            <tr className="font-bold">
                                <th className="border border-black p-1 w-[5%]">ક્રમ</th>
                                <th className="border border-black p-1 w-[20%]">ચેક નંબર</th>
                                <th className="border border-black p-1">બેંક ખાતા ધારકનું નામ (Payee Name)</th>
                        ...
                        <tbody>
                            {billData.cheque_details.map((cheque, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="border border-black p-1">{cheque.cheque_number}</td>
                                    <td className="border border-black p-1">{cheque.payee_name}</td>
                                    <td className="border border-black p-1 text-right">₹ {formatRupees(cheque.amount)}</td>
                                    {/* *** CHANGE: Use formatDate on cheque.due_date *** */}
                                    <td className="border border-black p-1 text-center">{formatDate(cheque.due_date)}</td>
                                </tr>
                            ))}
                             <tr>
                                <td colSpan={3} className="border border-black p-1 text-right font-bold">કુલ ચેક રકમ (₹):</td>
                                <td className="border border-black p-1 text-right font-bold">₹ {formatRupees(netAmount)}</td>
                    ...
                    <div className="mb-6 text-sm consent-text">
                        ...
                        <p>
                            હું, નીચે સહી કરનાર ખેડૂત શ્રી {billData.farmer_name}, જાહેર કરું છું કે ઉપરોક્ત બીજ ઉત્પાદન કાર્યક્રમ અંતર્ગત થયેલ હિસાબ અને ઉપર દર્શાવેલ ચેક નં. {chequeNumbers} વાળા, કુલ રકમ ₹ {formatRupees(netAmount)} ના ચેક(કો) મને મળી ગયેલ છે, જે મને સંતોષકારક છે. હવે આ હિસાબ બાબતે મારો કોઇપણ જાતનો દાવો રહેતો નથી, અને મને આપેલ ચેક ની સંપૂર્ણ જવાબદારી મારી રેહસે. જેની નોંધ લેશો.
                        </p>
                        <p className="mt-2">(નોંધ: રુદ્ર સીડ્સ એકાઉન્ટ પે ચેક થી જ ચુકવણું કરશે.)</p>
                    </div>
            ...
            {/* Print Styles */}
            <style jsx global>{`
                .controls { /* Standard button styling from previous component */ }
                ...
                .bill-header p { margin-bottom: 2px; line-height: 1.3; }
                .bill-info span { display: inline-block; min-width: 45%; } /* Basic layout for info lines */
                .bill-table th, .bill-table td { text-align: left; vertical-align: top; }
                .bill-table .text-right { text-align: right; }
                .bill-table .text-center { text-align: center; }
                .consent-text p { margin-bottom: 0.5rem; line-height: 1.5; }
                .signature-area { border-top: 1px solid #ccc; padding-top: 5px; }
                ...
                @media print {
                  ...
                  .bill-container { margin: 0 !important; padding: 0 !important; border: none !important; width: auto !important; }
                  /* Make ONLY the bill div visible */
                  body > *:not(.bill-container) { display: none; }
                  ...
                  .bill-a4 {
                   ...
                   .bill-table th, .bill-table td { border-color: black !important; padding: 3px 5px;}
                   .bill-info span { min-width: 48%; } /* Adjust spacing slightly for print */
                   .signature-area { border-top: 1px solid black; }
                }
