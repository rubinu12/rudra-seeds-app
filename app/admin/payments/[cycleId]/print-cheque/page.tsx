...
import { getChequePrintDetails } from '@/lib/payment-data'; // Adjust path if needed
  ...
  if (!printData || !printData.cheques || printData.cheques.length === 0) {
     return (
        <div className="p-8 max-w-2xl mx-auto text-center">
             <Link href={`/admin/payments/${cycleId}/process`} className="inline-flex items-center text-primary hover:underline mb-4">
                <ArrowLeft className="w-4 h-4 mr-2" /> Back to Payment Processing
            </Link>
            <h1 className="text-2xl font-semibold mb-4 text-error">Cheque Data Not Found</h1>
            <p className="text-on-surface-variant">
                Could not retrieve valid cheque details for Cycle ID <span className='font-bold'>{cycleId}</span>.
                Please ensure payment has been processed correctly and the cycle status is 'Cheque Generated'.
            </p>
        </div>
    );
  }
