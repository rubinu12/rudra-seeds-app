...
import type { ChequePrintData, StoredChequeDetail } from '@/lib/payment-data';
...
function numberToWordsIn(num: number | string): { line1: string; line2: string } {
    ...
    for (const word of wordsArray) {
        if ((currentLine + word).length + (currentLine ? 1 : 0) > charLimit && !line1) { line1 = currentLine.trim(); currentLine = word + ' '; } else { currentLine += word + ' '; }
    }
...
export default function PrintChequeClient({ printData }: PrintChequeClientProps) {
    ...
    return (
        <>
            ...
            <div className={`controls p-4 max-w-md mx-auto my-4 border rounded shadow bg-white print:hidden ${isPrinting ? 'invisible' : ''}`}>
                ...
                <div className="mb-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" className="h-4 w-4 rounded text-primary border-outline focus:ring-primary" checked={includeAcPayee} onChange={(e) => setIncludeAcPayee(e.target.checked)} />
                        Include A/C Payee Marking
                    </label>
                </div>
                 {totalCheques > 1 && (
                    <div className="flex justify-between mb-3">
                         <button onClick={goToPrevCheque} className="btn text-sm border border-outline text-primary px-4 py-2 rounded-full inline-flex items-center gap-1 hover:bg-primary/10"> <ArrowLeft size={16} /> Prev </button>
                         <button onClick={goToNextCheque} className="btn text-sm border border-outline text-primary px-4 py-2 rounded-full inline-flex items-center gap-1 hover:bg-primary/10"> Next <ArrowRight size={16} /> </button>
                    </div>
                 )}
                <button onClick={handlePrint} className="w-full bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 flex items-center justify-center gap-2"> <Printer size={18}/> Print Current Cheque </button>
                 <button onClick={() => window.history.back()} className="w-full bg-gray-200 text-gray-700 py-2 px-4 rounded hover:bg-gray-300 mt-2"> Back </button>
            </div>

            {/* Cheque Rendering Section (Only the current cheque) */}
            <div className="cheque-container"> {/* Container helps with print isolation */}
                <div className="cheque"> {/* This div has the exact dimensions */}
                    {includeAcPayee && (
            ...
            <style jsx global>{`
                ...
                .controls h2 { font-size: 1.1rem; font-weight: 600; margin-bottom: 1rem; }
                .controls label { display: flex; align-items: center; gap: 0.5rem; cursor: pointer; margin-bottom: 1rem; }
                .controls input[type="checkbox"] { margin-right: 8px; }
                .controls button { width: 100%; padding: 10px; background-color: #0070f3; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; margin-bottom: 0.5rem; display: inline-flex; justify-content: center; align-items: center; gap: 0.5rem;}
                .controls button:last-child { background-color: #e5e7eb; color: #374151; }
                .controls .flex { display: flex; justify-content: space-between; margin-bottom: 1rem; }
                .controls .btn { width: auto; background-color: white; color: #0070f3; border: 1px solid #ccc; padding: 8px 16px; font-size: 14px;}
                ...
                .cheque {
                    ...
                    /* EXACT dimensions */
                    width: 205mm; height: 92mm;
                    overflow: hidden; /* Important */
                 }

                /* EXACT field positioning and styles from cheque-test */
                .date-field { position: absolute; top: 7mm; left: 160mm; letter-spacing: 3mm; font-weight: bold; font-size: 14px; } /* Adjusted font size */
                .payee-field { position: absolute; top: 19mm; left: 19mm; font-weight: bold; font-size: 14px; } /* Adjusted font size */
                .amount-words-1 { position: absolute; top: 28mm; left: 30mm; font-size: 14px; } /* Adjusted font size */
                .amount-words-2 { position: absolute; top: 36mm; left: 15mm; font-size: 14px; } /* Adjusted font size */
                .amount-figures { position: absolute; top: 35mm; left: 158mm; font-weight: bold; font-size: 14px; } /* Adjusted font size */
                .ac-payee-container { position: absolute; top: 15mm; left: 3mm; transform: rotate(-45deg); transform-origin: top left; }
                .ac-payee-text-with-lines { border-top: 0.5mm solid #333; border-bottom: 0.5mm solid #333; padding: 1mm 0; width: 25mm; text-align: center; font-weight: bold; font-size: 12px; color: #333; }
                ...
                @media print {
                  ...
                  .cheque-container { margin: 0; padding: 0; border: none; }
                  /* Make ONLY the cheque div visible and position it */
                  body > *:not(.cheque-container) { display: none; }
                  html, body { height: auto; } /* Allow content height */
                  .cheque { position: absolute !important; top: 0 !important; left: 0 !important; margin: 0 !important; border: none !important; box-shadow: none !important;}
                }
