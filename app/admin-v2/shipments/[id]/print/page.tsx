...
import { getShipmentBillData } from '@/app/admin/actions/adminShipment';
...
export default function ShipmentBillPage() {
        ...
        if (id) {
            getShipmentBillData(id).then((res: any) => {
                if (res) {
                    ...
                    setDate(res.shipment.dispatch_date 
    ...
    if (loading || !shipmentData) return <div className="h-screen flex items-center justify-center"><LoaderCircle className="animate-spin w-8 h-8 text-gray-400" /></div>;

    const totalWeight = rows.reduce((sum, row) => sum + row.weight, 0);
    const grandTotalAmount = rows.reduce((sum, row) => sum + row.amount, 0);
    const varietyString = shipmentData.varieties && shipmentData.varieties.length > 0 ? shipmentData.varieties.join(", ") : "N/A";

    return (
        <div className="min-h-screen bg-gray-100 p-8 print:p-0 print:bg-white font-sans text-black">
            
            {/* CSS to Hide Number Spinner */}
            <style jsx global>{`
            ...
            <div className="max-w-[210mm] mx-auto mb-6 flex justify-between items-center print:hidden">
                <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-600 hover:text-black font-bold transition-colors">
                    <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <div className="flex items-center gap-2">
                    <button onClick={() => window.print()} className="bg-black text-white px-5 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-gray-800 shadow-md">
            ...
            {/* --- A4 PAPER CONTAINER --- */}
            <div className="max-w-[210mm] mx-auto bg-white shadow-2xl print:shadow-none p-[10mm] min-h-[297mm] text-sm relative leading-normal">
                
                {/* 1. TOP HEADER (Logo/Address) */}
                <div className="text-center border-b-2 border-black pb-2 mb-4">
                    <h1 className="text-3xl font-black tracking-wider uppercase mb-1">RUDRA SEED & ORGANISER</h1>
                    <p className="font-bold text-gray-800 text-xs">Shop No. 16, Nr. Sardar Chok, Jetpur Road, Dhoraji - 360 410</p>
                    <p className="font-bold text-gray-800 text-xs">Denish Patel: Mo: 8154000459 | GST No: 24DNTPG8564E1ZL</p>
                </div>

                {/* 2. TITLE (Centered) */}
                <div className="text-center mb-[12mm]">
                    <h2 className="text-xl font-black uppercase inline-block border-b-2 border-black tracking-wide">DISPATCH NOTE</h2>
                </div>
                ...
                <div className="flex justify-between items-start mb-6 w-full">
                    ...
                    <div className="w-1/2 text-left space-y-1.5">
                        <div className="flex items-baseline gap-3">
                            ...
                            <span className="font-bold text-base uppercase">{shipmentData.dest_name}</span>
                        </div>
                        <div className="flex items-baseline gap-3">
                ...
                <div className="mb-4 space-y-2 text-sm">
                    <p className="font-bold border-b border-gray-300 pb-1 inline-block">Subject: Dispatch of Certified Seed Bags</p>
                    
                    <p className="mt-1"><span className="font-bold">Variety:</span> {varietyString}</p>
                    
                    <p className="text-justify leading-relaxed">
                        Total <span className="font-bold">{shipmentData.total_bags}</span> certified seed bags of <span className="font-bold">{varietyString}</span> have been dispatched today ({date}) from <span className="font-bold">{fromLoc}</span> to your company, <span className="font-bold">{shipmentData.dest_name}</span> at <span className="font-bold uppercase">{atCity || "_______"}</span>.
                    </p>
                    
                    <p className="text-xs italic text-gray-600">
                        * This dispatch is carried out in accordance with the rules and standards of the Gujarat State Seed Certification Agency. Please accept delivery and handle it with care.
                    </p>
                    <p className="mt-2 font-bold underline text-xs">The list of sub-seed producers is as follows:</p>
                </div>
                ...
                <table className="w-full border-collapse border border-black mb-6 text-center text-xs">
                    <thead>
                        <tr className="bg-gray-100 print:bg-gray-100">
                            ...
                            <th className="border border-black p-1 text-left pl-2">Sub Seed Producer Name</th>
                            ...
                            <th className="border border-black p-1 w-16 bg-blue-50 print:bg-transparent">Rate</th>
                        ...
                        {rows.map((row, idx) => (
                            <tr key={idx}>
                                ...
                                <td className="border border-black p-1 text-left pl-2 font-bold whitespace-nowrap">
                                    {row.farmer_name} <span className="text-[10px] font-normal text-gray-600 ml-1">({row.village_name})</span>
                                </td>
                                ...
                                <td className="border border-black p-1 text-right pr-2 font-medium">{row.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                            </tr>
                        ))}
                        {/* TOTAL ROW */}
                        <tr className="font-bold bg-gray-100 print:bg-gray-100">
                            <td className="border border-black p-1.5 text-right pr-2" colSpan={3}>Total</td>
                            <td className="border border-black p-1.5">{shipmentData.total_bags}</td>
                            <td className="border border-black p-1.5">{totalWeight}</td>
                            <td className="border border-black p-1.5">-</td>
                            <td className="border border-black p-1.5 text-right pr-2 text-sm">{grandTotalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div className="border border-black p-3 mb-10 text-xs font-medium">
                    <div className="grid grid-cols-2 gap-x-16 gap-y-1.5">
                        ...
                        <div className="flex justify-between border-b border-dotted border-gray-400 pb-0.5">
                            ...
                            <span className="font-bold uppercase">{shipmentData.vehicle_number}</span>
                        </div>
                        ...
                        <div className="flex justify-between border-b border-dotted border-gray-400 pb-0.5">
                            ...
                            <span className="font-bold uppercase">{shipmentData.driver_name}</span>
                        </div>
                        ...
                        <div className="flex justify-between border-b border-dotted border-gray-400 pb-0.5 items-center">
                            <span className="font-bold text-gray-700">Vehicle Fare (₹) :</span>
                            <input 
                                value={fare} 
                ...
                <div className="flex justify-between mt-auto px-12 pt-8">
                    <div className="text-center w-40">
                        <p className="font-bold border-t border-black pt-2 text-xs">Sender Signature</p>
                    </div>
                    <div className="text-center w-40">
                        <p className="font-bold border-t border-black pt-2 text-xs">Receiver Signature</p>
                    </div>
                </div>
