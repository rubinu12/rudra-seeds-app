...
import { PlusCircle, Sprout, FileJson, Search, RefreshCw, MapPin } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { getSowingData, SowingEntry } from '@/app/admin/actions/lotManagement';
import LotNumberModal from '@/components/admin/sowing/LotNumberModal'; 
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-white rounded-3xl p-8 border border-gray-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">Sowing Operations</h2>
            <p className="text-gray-500 text-sm">Manage sowing cycles and seed lots for {seasonLabel}</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="p-5 border-b border-gray-100 flex gap-4 bg-gray-50/50">
              ...
              <button onClick={loadData} className="p-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-slate-600">
              ...
              <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                      {loading ? (
                          ...
                          <tr><td colSpan={5} className="p-10 text-center text-gray-400 italic">No sowing records found</td></tr>
                      ) : (
                          filtered.map((row) => (
                              <tr key={row.crop_cycle_id} className="hover:bg-green-50/30 transition-colors">
                                  <td className="p-5 font-bold text-gray-800">{row.farmer_name}</td>
                                  ...
                                  <td className="p-5 text-gray-600 font-medium">{row.sowing_date}</td>
                                  <td className="p-5">
                                      {row.lot_no ? (
