...
import { Plus, Truck, SearchX, Briefcase, Leaf, MapPin, ChevronDown, ChevronUp, AlertCircle } from "lucide-react";
import { getActiveShipments, ActiveShipment } from "@/app/employee-v2/actions/shipments";
import NewShipmentModal from "@/components/employee/loading/NewShipmentModal"; 

export default function LoadTab({ location }: { location: string }) {
    ...
    const loadData = async () => {
        setLoading(true);
        // Fetch ALL active trucks (no location arg needed now)
    ...
    return (
        <div className="space-y-4 pb-24 animate-in fade-in">
            ...
            {loading ? (
                <div className="text-center py-10 text-slate-400">Checking active trucks...</div>
            ) : (
                <>
                    {/* PRIMARY LIST (Assigned) */}
                    {assignedList.length > 0 ? (
                        assignedList.map(s => <TruckCard key={s.shipment_id} s={s} onClick={() => router.push(`/employee-v2/shipment/${s.shipment_id}`)} />)
                    ) : (
                        ...
                        <div className="pt-2">
                            ...
                            >
                                <span className="text-xs font-bold uppercase flex items-center gap-2">
                                    <AlertCircle className="w-4 h-4" /> Other Yards ({otherList.length})
                                </span>
                            ...
                            {isOthersOpen && (
                                <div className="mt-3 space-y-3 animate-in slide-in-from-top-2">
                                    {otherList.map(s => <TruckCard key={s.shipment_id} s={s} onClick={() => router.push(`/employee-v2/shipment/${s.shipment_id}`)} />)}
                                </div>
                            )}
                        </div>
                    )}
...
function TruckCard({ s, onClick }: { s: ActiveShipment, onClick: () => void }) {
    ...
    return (
        <div 
            onClick={onClick}
            className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm active:scale-[0.98] transition-all cursor-pointer relative overflow-hidden"
        >
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="font-bold text-slate-900 text-xl">{s.vehicle_number}</h3>
                ...
                <div className="text-right">
                    <span className={`text-2xl font-black ${isOver ? 'text-orange-500' : 'text-slate-900'}`}>
                        {Math.abs(remaining)}
                    </span>
            ...
            <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden mb-2">
                <div 
                    className={`h-full transition-all duration-500 ${isOver ? 'bg-orange-500' : 'bg-blue-600'}`}
                    style={{ width: `${Math.min((s.total_bags / s.target_bag_capacity) * 100, 100)}%` }}
                />
            </div>
