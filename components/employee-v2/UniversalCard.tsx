...
export default function UniversalCard({ data, location, onAction }: any) {
  ...
  // FIX: Initialize with the global location passed from props, NOT hardcoded 'Farm'
    ...
    switch (data.status) {
            ...
            <div className="flex-grow">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold py-2 pl-3 pr-8 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-slate-200"
                >
      ...
      case "Harvested":
        return (
          <button
            onClick={handleAction}
            className={`w-full py-3 mt-3 rounded-xl font-bold text-sm text-white shadow-md active:scale-95 transition-transform flex items-center justify-center gap-2 ${theme.btn}`}
          >
            <CheckCircle2 className="w-4 h-4" />
            {GUJARATI.btn_mark_collected}
          </button>
        );
      ...
      case "Sample Collected":
        return (
          <div className="flex flex-col gap-2 mt-3">
            ...
            <button
              onClick={handleAction}
              className="w-full py-3 rounded-xl font-bold text-sm text-white shadow-md active:scale-95 transition-transform flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 border border-purple-500"
            >
              <ClipboardList className="w-4 h-4" />
              {GUJARATI.btn_enter_lab_data}
            </button>
          </div>
        );
      ...
      case "Priced":
        return (
          <button
            ...
            className={`w-full py-3 mt-3 rounded-xl font-bold text-sm text-white shadow-md active:scale-95 transition-transform flex items-center justify-center gap-2 ${theme.btn}`}
          >
            <Scale className="w-4 h-4" /> {GUJARATI.btn_start_weighing}
          </button>
        );
      ...
      case "Weighed":
        return (
          <button
            ...
            className={`w-full py-3 mt-3 rounded-xl font-bold text-sm text-white shadow-md active:scale-95 transition-transform flex items-center justify-center gap-2 ${theme.btn}`}
          >
            <Truck className="w-4 h-4" /> {GUJARATI.btn_add_to_shipment}
          </button>
        );
  ...
  return (
    <div
      className="bg-white rounded-[16px] p-4 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.1)] border border-slate-100 relative overflow-hidden mb-3"
      style={cardStyle}
    >
