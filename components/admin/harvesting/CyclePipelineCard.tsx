...
import { CyclePipelineStatus } from '@/lib/admin-data'; // Import the type
import { ArrowRightCircle, Package, ThumbsUp, Scale, ClipboardCheck } from 'lucide-react';
  ...
  return (
    <div className="bg-surface-container rounded-3xl p-6 shadow-sm relative">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-xl font-medium text-on-surface">Pipeline Status</h3>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-5 mt-6">
        <PipelineItem icon={ThumbsUp} label="Harvested" value={currentData.harvested} color="text-blue-500" />
        <PipelineItem icon={ClipboardCheck} label="Sampled" value={currentData.sampled} color="text-purple-500" />
        <PipelineItem icon={Package} label="Priced" value={currentData.priced} color="text-amber-500" />
        <PipelineItem icon={Scale} label="Weighed" value={currentData.weighed} color="text-teal-500" />
      </div>
    </div>
  );
}
...
const PipelineItem = ({ icon: Icon, label, value, color }: { icon: React.ElementType, label: string, value: number, color: string }) => (
