...
import { DispatchedShipmentInfo } from '@/lib/shipment-data'; // Use the new path
import { Truck, CalendarDays, ChevronRight, FileText, LoaderCircle } from 'lucide-react';
...
export default function SelectShipmentBillModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Select Shipment to Generate Bill" maxWidth="max-w-lg">
            ...
            >
              ...
              <div className="flex-grow min-w-0 text-left">
                <p className="font-semibold text-on-surface text-base truncate flex items-center gap-1.5">
                  ...
                  {shipment.vehicle_number || `Shipment #${shipment.shipment_id}`}
                </p>
                ...
                <p className="text-xs text-on-surface-variant mt-1 flex items-center gap-1">
                   ...
                   Dispatched: {shipment.dispatch_date || 'N/A'} ({shipment.total_bags || 0} Bags)
                </p>
              </div>
              {/* Right side action */}
              <div className="flex items-center gap-1 text-primary flex-shrink-0">
                {isNavigatingId === shipment.shipment_id ? (
                    ...
                    <>
                        <span className="text-sm font-medium hidden sm:inline group-hover:underline">Generate Bill</span>
