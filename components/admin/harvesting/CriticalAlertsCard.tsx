...
import { CriticalAlertsData } from '@/lib/admin-data'; // Import the type
  ...
  return (
    <div className="bg-error-container rounded-3xl p-6 shadow-sm relative text-on-error-container">
      ...
      <div className="grid grid-cols-1 gap-5 mt-6">
        ...
        >
          <Timer className="w-6 h-6 text-error flex-shrink-0" strokeWidth={1.5} />
          <div>
            <p className="text-2xl font-semibold">{data.pricedOver12DaysNotWeighed}</p>
            <p className="text-xs font-medium">Priced 12 days, Weighing Pending</p>
          </div>
        </button>
        ...
        <button
          ...
          className="flex items-center gap-3 p-4 rounded-xl bg-error/20 hover:bg-error/30 transition-colors w-full text-left"
        >
          <Package className="w-6 h-6 text-error flex-shrink-0" strokeWidth={1.5} />
          <div>
