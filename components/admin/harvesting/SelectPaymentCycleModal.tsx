...
import { CycleForPaymentSelection } from '@/lib/payment-data'; // Adjust path if needed
import { Wheat, User, Tag, Package, ChevronRight, LoaderCircle, IndianRupee } from 'lucide-react';
...
export default function SelectPaymentCycleModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Select Cycle for Farmer Payment" maxWidth="max-w-xl"> {/* Adjusted width */}
      <div className="space-y-3">
        {isLoading ? (
          <div className="flex justify-center items-center py-8">
            <LoaderCircle className="w-6 h-6 animate-spin text-primary" />
            <p className="ml-2 text-on-surface-variant">Loading cycles ready for payment...</p>
          </div>
        ) : cycles.length > 0 ? (
          cycles.map((cycle) => (
            <button
              ...
              <div className="flex-grow min-w-0 text-left space-y-1">
                ...
                <div className="flex items-center gap-4 text-xs text-on-surface-variant">
                    ...
                    <span className="flex items-center gap-1">
                        <Package className="w-3 h-3"/> {cycle.quantity_in_bags || 0} Bags
                    </span>
                </div>
              </div>
              {/* Right side action */}
              <div className="flex items-center gap-1 text-primary flex-shrink-0">
                {isNavigatingId === cycle.crop_cycle_id ? (
                    ...
                    <>
                        ...
                        <span className="text-sm font-medium hidden sm:inline group-hover:underline">Process Payment</span>
