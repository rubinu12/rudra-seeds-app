...
// Define the shape for the options arrays
type Option = { value: string; label: string; };

// Update Props to accept the formatted options
type Props = {
...
export const SowingSection = ({ cycleState, landmarkOptions, seedVarietyOptions }: Props) => {
    ...
    // A generic handler for simple value changes from inputs
    const handleValueChange = (e: React.ChangeEvent<any>) => {
        const { name, value, type } = e.target;
        setCycleData((prev: any) => ({ ...prev, [name]: type === 'number' ? Number(value) : value }));
    };

    // A specific handler for our custom searchable select component
        ...
        <div className="bg-surface-container rounded-[1.75rem] p-6 shadow-md">
          <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 grid place-items-center rounded-2xl bg-primary-container">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#21005D" strokeWidth="2"><path d="M17.5 2.5a2.5 2.5 0 0 1 0 5L16 6l-2.5 2.5L15 10l-2.5 2.5L14 14l-2.5 2.5L13 18l-2.5 2.5L9 19l-2.5-2.5L8 15l-2.5-2.5L7 11l-2.5-2.5L6 7l2.5-2.5L10 6Z"></path></svg>
              </div>
              ...
              <div>
                  ...
                  <div className="flex items-center gap-6">
                      <Radio 
                        id="pickup" 
                        name="collection" 
                        value="RudraSeeds Pickup" 
                        label="RudraSeeds Pickup"
                        checked={cycleData.collection === 'RudraSeeds Pickup'}
                        onChange={handleValueChange}
                      />
                      <Radio 
                        id="dropoff" 
                        name="collection" 
                        value="Farmer Drop-off" 
                        label="Farmer Drop-off"
                        checked={cycleData.collection === 'Farmer Drop-off'}
                        onChange={handleValueChange}
                      />
                  </div>
              </div>
