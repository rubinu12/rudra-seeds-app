...
export async function getShipmentBillData(shipmentId: number) {
  try {
    // 1. Fetch Shipment Header Details
    ...
    // 2. Fetch Item Details directly from shipment_items!
    const itemsRes = await sql`
        SELECT 
            f.name as farmer_name,
            COALESCE(v.village_name, '') as village_name,
            
            si.lot_no,               -- FETCHING DIRECTLY FROM SHIPMENT_ITEMS
            si.bags_loaded as bags,  -- FETCHING EXACT LOADED AMOUNT
        ...
        ORDER BY f.name, se.variety_name, si.lot_no
    `;
