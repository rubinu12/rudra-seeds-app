// src/app/admin/shipments/ShipmentClient.tsx
"use client";
...
function TabButton({ active, onClick, label, icon, count }: TabButtonProps) {
  return (
    <button
      onClick={onClick}
...
function ShipmentCard({
  ...
  // [NEW] Logic to check if Bill is already generated
  const isBillGenerated = data.status === 'Bill Generated';

  return (
    <div 
        className={`
            bg-white border rounded-2xl overflow-hidden shadow-sm transition-all flex flex-col relative
            ${isBillGenerated ? 'border-green-100 opacity-60 hover:opacity-100' : 'border-slate-200 hover:shadow-md'}
        `}
    >
      {/* Header Status Strip */}
      <div
        className={`h-2 w-full ${
          type === "pending" ? "bg-orange-400" : (isBillGenerated ? "bg-slate-300" : "bg-green-500")
        }`}
      />
      
      {/* [NEW] Badge for Bill Generated */}
      {isBillGenerated && (
        <div className="absolute top-4 right-4 bg-green-100 text-green-700 px-2 py-1 rounded-md text-[10px] font-bold flex items-center gap-1">
            <FileCheck className="w-3 h-3" /> Billed
        </div>
      )}
      ...
      <div className="p-5 flex-1">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          {/* Only show bag count if not Billed (to reduce visual noise) or keep it if preferred */}
        ...
        <div className="bg-slate-50 rounded-xl p-3 space-y-2 text-xs mb-4">
          ...
          {type === "history" && data.dispatch_date && (
      ...
      <div className="p-4 pt-0 mt-auto flex gap-2">
        {type === "pending" ? (
          ...
          >
            ...
            {isBillGenerated ? "Reprint Bill" : "Print Bill"}
          </button>
        )}
      </div>
    </div>
